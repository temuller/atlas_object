...
def calc_sigma(data, errors=None):
    ...
    wsigma = np.sqrt(
        ((data - wmean) * (data - wmean) * w).sum() * (V1 / (V1 * V1 - V2))
    )

    return wmean, wsigma


def weighted_sigmaclip(
    data, errors=None, niter=1, n_sigma=3, use_median=False
):
    ...
    indices = np.ones(len(data.ravel()), dtype=np.bool).reshape(data.shape)
